...
mod_xephyr () {
	...
	mod_xephyr_run_bspwm "$1" "$2"

}


mod_xephyr_run_bspwm () {
	...
	if [ "none$config_file_path" = "none" ]; then
		util_error_echo "DISPLAY=:100 bspwm"
		DISPLAY=:100 bspwm
	else
		util_error_echo "DISPLAY=:100 bspwm -c $config_file_path"
		DISPLAY=:100 bspwm -c $config_file_path
	fi
